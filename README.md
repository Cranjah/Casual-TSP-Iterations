...
**01-iterate-tsp.py** is my first naively implemented and logically scaleable - but not runtime scaleable - solution to the TSP with mainly a permutating brute-force approach and different list operations - written in Python. The asymptotic runtime would be O(n!), which is a suboptimal solution in terms of efficiency and far away from solution to the overall P-NP-Problem.
